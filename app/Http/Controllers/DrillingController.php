...
//         'machine_start'     => 'nullable',
//         'machine_end'       => 'nullable',
//         'actual_machine_hr' => 'nullable|numeric',
...
//         'message' => 'Drilling record with work points and survey details stored successfully!',
//         'data'    => $drillingRecord->load('workPoints', 'surveys'),
//         'summary' => $summary
//     ], 201);
// }
// public function store(Request $request)
...
// // ✅ Step 2.3: Deduct Raw Material Stock & Save Usage Record
// //  $usedRawMaterials = [];
// // if ($request->has('raw_material_usage') && is_array($request->raw_material_usage)) {
// //     foreach ($request->raw_material_usage as $usage) {
// //         // usage should contain: material_id and qty_used
// //         $materialId = $usage['material_id'] ?? null;
// //         $qtyUsed    = $usage['qty_used'] ?? 0;
// //         $misc       = $usage['misc'] ?? null; // optional misc data

// //         if ($materialId && $qtyUsed > 0) {
// //             // Find latest raw material log for this material and project
...
// //             if ($rawLog) {
// //                 // ✅ Deduct stock (but not below zero)
// //                 $newStock = max(0, $rawLog->current_stock - $qtyUsed);
// //                 $rawLog->update(['current_stock' => $newStock]);
// //             }

// //             // ✅ Store usage in uses_raw_material table
...
// //                 // Deduct total amount (total column) (but not below zero)
...
// //                 'material_id' => $materialId,
// //                 'used_qty'    => $qtyUsed,
// //                 'price'       => $rawLog->price ?? 0, // store unit price
// //                 'amount'      => $amountUsed,         // store total cost
// //                 'misc'        => $misc,
// //             ]);
...
// class DrillingRecordController extends Controller
// {
    // public function store(Request $request)
    // {
    ...
    //     // Step 1: Validate request fields (all optional)
    //     $validated = $request->validate([
    //         'project_id'        => 'nullable|integer',
    //         'date'              => 'nullable|date',
    //         'oprator_helper'    => 'nullable|integer', // Assuming it references Operator ID
    //         'machine_start'     => 'nullable|numeric',
    //         'machine_end'       => 'nullable|numeric',
    //         'actual_machine_hr' => 'nullable|numeric',
    //         'comp_rpm_start'    => 'nullable|numeric',
    //         'comp_rpm_end'      => 'nullable|numeric',
    //         'com_actul_hr'      => 'nullable|numeric',
    //         'machineReading'    => 'nullable|array',
    //         'machineReading.*.oprator_id' => 'nullable|integer',
    //         'machineReading.*.machine_id' => 'nullable|integer',
    //         'machineReading.*.machine_start' => 'nullable|numeric',
    //         'machineReading.*.machine_end' => 'nullable|numeric',
    //         'machineReading.*.actual_machine_hr' => 'nullable|numeric',
    //         'compressor_rpm'    => 'nullable|array',
    //         'compressor_rpm.*.oprator_id' => 'nullable|integer',
    //         'compressor_rpm.*.machine_id' => 'nullable|integer',
    //         'compressor_rpm.*.comp_rpm_start' => 'nullable|numeric',
    //         'compressor_rpm.*.comp_rpm_end' => 'nullable|numeric',
    //         'compressor_rpm.*.com_actul_hr' => 'nullable|numeric',
    //         'workPoints'        => 'nullable|array',
    //         'workPoints.*.details' => 'nullable|string', // Adjust based on WorkPointDetail schema
    //         'surveys'           => 'nullable|array',
    //         'surveys.*.details' => 'nullable|string', // Adjust based on SurveyDetail schema
    //         'uses_raw_material' => 'nullable|array',
    //         'uses_raw_material.*.material_id' => 'nullable|integer', // Adjust based on UsesRawMaterial schema
    //         'uses_raw_material.*.quantity' => 'nullable|numeric', // Example field
    //     ]);

    //     // Step 2: Create DrillingRecord with authenticated user data
    //     $drillingRecord = DrillingRecord::create([
    //         'company_id'        => $companyId,
    //         'user_id'           => $userId,
    //         'project_id'        => $validated['project_id'] ?? null,
    //         'date'              => $validated['date'] ?? null,
    //         'oprator_helper'    => $validated['oprator_helper'] ?? null,
    //         'machine_start'     => $validated['machine_start'] ?? null,
    //         'machine_end'       => $validated['machine_end'] ?? null,
    //         'actual_machine_hr' => $validated['actual_machine_hr'] ?? null,
    //         'comp_rpm_start'    => $validated['comp_rpm_start'] ?? null,
    //         'comp_rpm_end'      => $validated['comp_rpm_end'] ?? null,
    //         'com_actul_hr'      => $validated['com_actul_hr'] ?? null,
    //     ]);

    //     // Step 3: Create MachineReading entries
    //     if ($request->has('machineReading') && is_array($request->machineReading)) {
    //         foreach ($request->machineReading as $reading) {
    //             MachineReading::create([
    //                 'company_id'        => $companyId,
    ...
    //     // Step 4: Create CompressorRpm entries
    //     if ($request->has('compressor_rpm') && is_array($request->compressor_rpm)) {
    //         foreach ($request->compressor_rpm as $rpm) {
    //             CompressorRpm::create([
    //                 'company_id'         => $companyId,
    ...
    //     // Step 5: Create WorkPointDetail entries
    //     if ($request->has('workPoints') && is_array($request->workPoints)) {
    //         foreach ($request->workPoints as $workPoint) {
    //             WorkPointDetail::create([
    //                 'company_id'         => $companyId,
    //                 'drilling_record_id' => $drillingRecord->id,
    //                 'user_id'            => $userId,
    //                 'details'            => $workPoint['details'] ?? null, // Adjust based on schema
    //             ]);
    //         }
    //     }

    //     // Step 6: Create SurveyDetail entries
    //     if ($request->has('surveys') && is_array($request->surveys)) {
    //         foreach ($request->surveys as $survey) {
    //             SurveyDetail::create([
    //                 'company_id'         => $companyId,
    //                 'drilling_record_id' => $drillingRecord->id,
    //                 'user_id'            => $userId,
    //                 'details'            => $survey['details'] ?? null, // Adjust based on schema
    //             ]);
    //         }
    //     }

    //     // Step 7: Create UsesRawMaterial entries
    //     if ($request->has('uses_raw_material') && is_array($request->uses_raw_material)) {
    //         foreach ($request->uses_raw_material as $material) {
    //             UsesRawMaterial::create([
    //                 'company_id'         => $companyId,
    //                 'drilling_record_id' => $drillingRecord->id,
    //                 'user_id'            => $userId,
    //                 'material_id'        => $material['material_id'] ?? null,
    //                 'quantity'           => $material['quantity'] ?? null, // Adjust based on schema
    //             ]);
    //         }
    //     }
...
public function store(Request $request)
{
    ...
    // Validation (compressor_rpm removed as a table)
    $validated = $request->validate([
        'project_id'        => 'nullable|integer|exists:projects,id',
        'date'              => 'nullable|date',
        'oprator_helper'    => 'nullable|integer|exists:operators,id',
        'machine_start'     => 'nullable|numeric',
        'machine_end'       => 'nullable|numeric',
        'comp_rpm_start'    => 'nullable|numeric',
        'comp_rpm_end'      => 'nullable|numeric',
        ...
        'machineReading'    => 'nullable|array',
        'machineReading.*.operator_id' => 'nullable|integer|exists:operators,id',
        'machineReading.*.machine_id' => 'nullable|integer|exists:machine_operator,id',
        'machineReading.*.machine_start' => 'nullable|numeric',
        'machineReading.*.machine_end' => 'nullable|numeric',

        'workPoints'        => 'nullable|array',
        'workPoints.*.details' => 'nullable|string',

        'surveys'           => 'nullable|array',
        'surveys.*.details' => 'nullable|string',

        'uses_raw_material' => 'nullable|array',
        'uses_raw_material.*.material_id' => 'nullable|integer|exists:materials,id',
        'uses_raw_material.*.quantity' => 'nullable|numeric',
    ]);

    DB::beginTransaction();
    try {
        // Create DrillingRecord
        $drillingRecord = DrillingRecord::create([
            'company_id'     => $companyId,
            'user_id'        => $userId,
            'project_id'     => $validated['project_id'] ?? null,
        ...
        if ($request->has('machineReading') && is_array($request->machineReading)) {
            foreach ($request->machineReading as $reading) {
                MachineReading::create([
                    'company_id'    => $companyId,
                    'user_id'       => $userId,
                    'operator_id'   => $reading['operator_id'] ?? null,
                    'project_id'    => $validated['project_id'] ?? null,
                    'machine_id'    => $reading['machine_id'] ?? null,
                    'machine_start' => $reading['machine_start'] ?? null,
                    'machine_end'   => $reading['machine_end'] ?? null,
        ...
        if ($request->has('workPoints') && is_array($request->workPoints)) {
            foreach ($request->workPoints as $workPoint) {
                WorkPointDetail::create([
                    'company_id' => $companyId,
                    'user_id'    => $userId,
                    'details'    => $workPoint['details'] ?? null,
                ]);
            }
        }
        ...
        // Return response with compressor_rpm forced null
        return response()->json([
            'message' => 'Record created successfully',
            'data'    => $drillingRecord
                            ->load('machineReading', 'workPoints', 'surveys', 'uses_raw_material')
                            ->append(['compressor_rpm' => null]),
        ], 201);
...
//         ->where('company_id', $companyId) // ✅ filter by user's company
...
// -------------------------------
    // Company-wide data (for type=1)
    // -------------------------------
    // public function index(Request $request)
    // {
    //     $companyId = auth()->user()->company_id;

    //     $records = DrillingRecord::with(['surveyDetail', 'workPointDetail', 'project:id,project_name', 'Operator:id,name'])
