...
const Dashboard = (Props) => {
  ...
  const [summary, setSummary] = useState({
    machineCount: 0,
    totalProfit: 0, // kept for potential future use (machine earnings)
    totalExpenseAll: 0,
    totalNet: 0, // will be computed from totalSales - totalExpenseAll
    customerCount: 0,
    totalInvoices: 0,
    totalBilled: 0,
    totalPaid: 0,
  ...
  useEffect(() => {
    try {
      const fetchMonthlySales = async () => {
        const response = await getAPICall('/api/getMonthlyReportSummeries')
  ...
  // Format helpers (match report pages)
  ...
  // Compute company summary using the same APIs as reports
  useEffect(() => {
    ...
    const computeSummary = async () => {
      try {
        ...
        // Parallel API calls (as used in components)
        const [
          machineriesResp,
          machineLogsResp,
          expensesResp,
          ...
          getAPICall('/api/machineries1').catch(() => []),
          getAPICall(`/api/machineLog?company_id=${cid}`).catch(() => []),
          getAPICall(`/api/expense1?company_id=${cid}`).catch(() => [[]]),
          getAPICall(`/api/project-payments?company_id=${cid}`).catch(() => []),
          getAPICall(`/api/myProjects?company_id=${cid}`).catch(() => []),
        ]);

        const machineries = Array.isArray(machineriesResp) ? machineriesResp : [];
        const machineLogs = Array.isArray(machineLogsResp) ? machineLogsResp : [];
        const expensesRaw = Array.isArray(expensesResp)
          ? (Array.isArray(expensesResp[0]) ? expensesResp[0] : expensesResp)
          : [];
        const payments = Array.isArray(paymentsResp) ? paymentsResp : [];
        const totalProjects = Array.isArray(projects) ? projects.length : 0;
        // From MachineExpense.jsx
        const calcMachineProfit = (logs) => {
          return machineries.reduce((sum, m) => {
            const profit = logs
              .filter(log => Number(log.machine_id) === Number(m.id))
              .reduce((s, log) => {
                ...
                return s + ((end > start && price > 0) ? (end - start) * price : 0);
              }, 0);
            return sum + profit;
          }, 0);
        };
        ...
        const machineCount = machineries.length;
        const totalProfit = calcMachineProfit(machineLogs);
        const totalExpenseAll = calcAllExpenses(expensesRaw);

        // From ProjectPaymentReport.js
        const companyPayments = payments.filter(p => p.company_id === cid);
        const groupedByCustomer = companyPayments.reduce((acc, p) => {
        ...
        const nonAdvance = companyPayments.filter(p => !p.is_advance);
        const totalInvoices = nonAdvance.length;
        const totalBilled = nonAdvance.reduce((s, p) => s + Number(p.total || 0), 0);
        const totalPaid = nonAdvance.reduce((s, p) => s + Number(p.paid_amount || 0), 0);
        const totalOutstanding = totalBilled - totalPaid;
        const totalSales = totalBilled;
        const totalNet = totalSales - totalExpenseAll;

        // Build monthly summaries for chart
        const monthlySales = Array(12).fill(0);
        nonAdvance.forEach((payment) => {
          const rawDate = payment.created_at || payment.invoice_date;
          if (!rawDate) return;
          const parsed = new Date(rawDate);
          if (Number.isNaN(parsed.getTime())) return;
          const monthIdx = parsed.getMonth();
          monthlySales[monthIdx] += Number(payment.total || 0);
        });
        ...
        expensesRaw.forEach((expense) => {
          ...
          if (Number.isNaN(parsed.getTime())) return;
          const monthIdx = parsed.getMonth();
          monthlyExpense[monthIdx] += Number(expense.total_price || 0);
        });

        const monthlyPandL = monthlySales.map((sale, idx) => sale - monthlyExpense[idx]);
        const ytdPandL = monthlyPandL.reduce((sum, val) => sum + val, 0);
        ...
        showToast('danger', 'Failed to load dashboard summary');
      } finally {
        ...
        <CRow className="g-2 mb-3">
          ...
          {/* <CCol xs={6} md={3} lg={2}>
            <div className="rounded p-2 text-center bg-success text-white shadow-sm">
              <small className="d-block">Sales</small>
              <strong className="fs-6">{summaryLoading ? '-' : formatIndianNumber(summary.totalSales)}</strong>
            </div>
          </CCol>
          <CCol xs={6} md={3} lg={2}>
            <div className="rounded p-2 text-center bg-warning text-dark shadow-sm">
              <small className="d-block">Expenses</small>
