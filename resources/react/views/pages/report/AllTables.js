...
function All_Tables({
  ...
  const getCurrentData = () => {
    ...
    switch (selectedOption) {
      case '1':
        data = salesData?.data || [];
        console.log('Work Report Data:', data);
        ...
        switch (selectedOption) {
          case '1':
            return (
              ...
              (item.totalWorkAmount && item.totalWorkAmount.toString().includes(searchTerm))
            );
          case '2':
            return (
          ...
          case '3':
            return (
              (item.date && formatDate(item.date).toLowerCase().includes(searchTerm.toLowerCase())) ||
              (item.projectName && item.projectName.toLowerCase().includes(searchTerm.toLowerCase())) ||
              (item.totalWork && item.totalWork.toString().includes(searchTerm)) ||
  ...
  const getEmptyMessage = () => {
    ...
    switch (selectedOption) {
      case '1':
        return t('MSG.no_work_log_data') || 'No work log data available';
      case '2':
  ...
  const getSearchPlaceholder = () => {
    switch (selectedOption) {
      case '1':
        return t('LABELS.search_work_logs') || 'Search work log data...';
      case '2':
      ...
      case '1':
        return (
          <>
            ...
            <CTableHeaderCell onClick={() => handleSort('totalWorkAmount')} style={headerStyle}>
              {isMobile ? 'Amount' : (t('LABELS.total_work_amount') || 'Total Work Amount')}
              <span style={getSortIconStyle('totalWorkAmount')}>{getSortIcon('totalWorkAmount')}</span>
            </CTableHeaderCell>
          </>
        );
      case '2':
      ...
      case '3':
        return (
          <>
            ...
            <CTableHeaderCell onClick={() => handleSort('totalWork')} style={headerStyle}>
              {isMobile ? 'Work' : (t('LABELS.work_grand_total') || 'Work Total')}
              <span style={getSortIconStyle('totalWork')}>{getSortIcon('totalWork')}</span>
            </CTableHeaderCell>
          ...
          <>
            ...
            <CTableDataCell className="amount-cell">
              <span className="amount-value">
                {formatCurrency(item.totalWorkAmount)}
              </span>
            </CTableDataCell>
          </>
        )}
