 ...
 const handleDurationChange = (event) => {
  ...
  const prepareFormData = async () => {
    try {
      ...
      if (formData.subscribed_plan) {
      const selectedPlan = refData.plans.find(p => p.id == formData.subscribed_plan);
      if (selectedPlan && selectedPlan.name.toLowerCase().includes("free trial")) {
        finalData.next_plan_id = formData.next_plan_id;   // user must pick
      } else {
        finalData.next_plan_id = formData.subscribed_plan; // auto fallback
      }
    }
...
const registerCompanyDirectly = async () => {
  try {
    ...
    const companyResponse = await post('/api/companies', preparedFormData);
          ...
          <CForm noValidate validated={validated} onSubmit={handleSubmit} encType='multipart/form-data'>
            ...
            <div className='row'>
              ...
              <div className="col-sm-3">
                <div className="mb-3">
                  <CFormLabel htmlFor="billing_type">{t("LABELS.billing_type") || "Billing Type"}</CFormLabel>
                  <CFormSelect
                    id="billing_type"
                    name="billing_type"
                    ...
                    onChange={(e) => {
                      ...
                      setFormData((prev) => ({
                        ...
                        next_plan_id: '', // reset next plan also
                      }));
                    }}
                  >
                    <option value="">{t("LABELS.free_trial") || "-- Free Trial --"}</option>
            ...
            {/* ✅ Next Renewal section - visible only when Billing Type = Free Trial */}
            {selectedBillingType === "" && (
              <div className='row'>
                <div className="col-sm-4">
                  <div className="mb-3">
                    <CFormLabel htmlFor="next_billing_type">
                      {"Next Renewal Plan Type"}
                    </CFormLabel>
                    <CFormSelect
                      ...
                      onChange={(e) => {
                        ...
                        setFormData((prev) => ({ ...prev, next_plan_id: '' })); // reset
                      }}
                      required
                    >
                ...
                <div className="col-sm-4">
                  <div className="mb-3">
                    <CFormLabel htmlFor="next_plan_id">
                      {"Next Renewal Plan" || t("LABELS.next_renewal_pla") }
                    </CFormLabel>
                    <CFormSelect
                      id="next_plan_id"
                      name="next_plan_id"
                      ...
                      options={
                        refData.plans
                          .filter((plan) => {
                            const name = plan.name?.toLowerCase() || '';
                            return (
                              nextBillingType &&                 // ✅ match only when a type is chosen
                              name.includes(nextBillingType.toLowerCase()) && // ✅ match type
                              !name.includes("free trial")       // ✅ exclude free trial
                            );
                          })
                          .map((plan) => ({
                            value: plan.id,
