...
const fetchSubscriptionStatus = useCallback(async () => {
  try {
    const response = await getAPICall('/api/subscription/status');
    if (response) {
      setSubscriptionData(response);

      // ✅ only show modal if subscription is expired
    ...
    const [monthlyTrackers, setMonthlyTrackers] = useState([]);
    ...
    const fetchMonthly = useCallback(async () => {
        try {
            ...
            const res = await getAPICall(`/api/employee/trackers/monthly?year=${calendarYear}&month=${calendarMonth}`);
            if (res && res.days) {
                setMonthlyTrackers(Array.isArray(res.days) ? res.days : []);
                setDateOfJoining(res.date_of_joining || '');
            } else {
                setMonthlyTrackers(Array.isArray(res) ? res : []);
                setDateOfJoining('');
            }
        } catch (e) {
            console.error('Monthly trackers load error', e);
        } finally {
    ...
    const firstDay = useMemo(() => new Date(calendarYear, calendarMonth - 1, 1).getDay(), [calendarYear, calendarMonth]);

    const byDate = useMemo(() => {
        const map = {};
        for (const d of monthlyTrackers) { map[d.date] = d; }
        return map;
    }, [monthlyTrackers]);

    const dateToKey = (day) => `${calendarYear}-${String(calendarMonth).padStart(2,'0')}-${String(day).padStart(2,'0')}`;

    const renderedCells = useMemo(() => {
        const cells = [];
        for (let i = 0; i < firstDay; i++) cells.push(<div key={`e-${i}`} />);
        // Build string keys to avoid timezone issues (compare as YYYY-MM-DD)
        const now = new Date();
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            const isJoinDay = !!dateOfJoining && key === dateOfJoining;
            const isPreJoining = !!dateOfJoining && key < dateOfJoining;
            if (isFuture) {
                badge = <span className="text-muted">&nbsp;</span>;
            } else if (isJoinDay) {
                badge = <span className="badge bg-primary">DOJ</span>;
            } else if (isPreJoining) {
                // Pre-joining days: no absent marker, keep blank
                badge = <span className="text-muted">&nbsp;</span>;
            } else if (rec?.is_week_off) badge = <span className="badge bg-info text-dark">WWO</span>;
            else if (rec?.half_day) badge = <span className="badge bg-warning text-dark">HD</span>;
            else if (rec?.check_in) badge = <span className="badge bg-success">P</span>;
            else badge = <span className="badge bg-danger">A</span>;
            cells.push(
                <div key={key} className="border rounded text-center" style={{ padding: '2px', fontSize: 'clamp(10px, 2.8vw, 12px)' }}>
                    <div className="fw-semibold" style={{ fontSize: 'clamp(11px, 3vw, 13px)' }}>{d}</div>
                    <div>{badge}</div>
                </div>
            );
        }
        return cells;
    }, [firstDay, daysInMonth, byDate, dateOfJoining, calendarMonth, calendarYear]);

    const CalendarBlock = () => (
        <div className="p-3 border rounded">
            <h6 className="mb-2">{t('LABELS.monthlyAttendance') || 'Monthly Attendance'}</h6>
            <div className="d-flex align-items-center justify-content-between mb-2 flex-wrap gap-2">
                <div className="d-flex gap-2 align-items-center flex-wrap">
                    <select className="form-select form-select-sm" value={calendarMonth} onChange={e=>setCalendarMonth(parseInt(e.target.value))} style={{width: 120}}>
                        {Array.from({length:12},(_,i)=>i+1).map(m=> (<option key={m} value={m}>{['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][m-1]}</option>))}
                    </select>
                    <select className="form-select form-select-sm" value={calendarYear} onChange={e=>setCalendarYear(parseInt(e.target.value))} style={{width: 120}}>
                        {[calendarYear-1, calendarYear, calendarYear+1].map(y=> (<option key={y} value={y}>{y}</option>))}
                    </select>
                </div>
                <div>{loadingCalendar && <CSpinner size="sm" />}</div>
            </div>
            <div style={{ width: '100%', overflowX: 'hidden' }}>
                <div className="d-grid" style={{ gridTemplateColumns:'repeat(7, 1fr)', gap: '2px' }}>
                    {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d=> (<div key={d} className="text-center text-muted" style={{ fontSize: 'clamp(10px, 2.6vw, 12px)', padding: '2px' }}>{d}</div>))}
                    {renderedCells}
                </div>
            </div>
            <div className="d-flex gap-2 small mt-2 flex-wrap" style={{ fontSize: '0.8rem' }}>
                <span><span className="badge bg-success me-1">&nbsp;</span>{t('LABELS.present') || 'Present'}</span>
                <span><span className="badge bg-warning text-dark me-1">&nbsp;</span>{t('LABELS.halfDay') || 'Half Day'}</span>
                <span><span className="badge bg-info text-dark me-1">&nbsp;</span>{t('LABELS.workingWeekOff') || 'Working Week Off'}</span>
                <span><span className="badge bg-danger me-1">&nbsp;</span>{t('LABELS.absent') || 'Absent'}</span>
                <span><span className="badge bg-primary me-1">&nbsp;</span>{t('LABELS.dateOfJoining') || 'Date of Joining'}</span>
            </div>
        </div>
    );
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    // Final fallback to user/front camera
                    stream = await navigator.mediaDevices.getUserMedia({
    ...
    const capturePhoto = useCallback(async () => {
        ...
        canvas.toBlob(async (blob) => {
            if (blob) {
                ...
                showNotification('success', t('MSG.photoCaptured') || 'Photo captured successfully');
            }
        }, 'image/jpeg', 0.8);
    ...
    }, [fetchEmployeeStatus, fetchSubscriptionStatus]);
        ...
        <div className="row g-2 g-sm-3 g-md-4 mb-3 mb-sm-4">
            <div className="col-12 col-sm-6">
                <CButton
                    ...
                    disabled={status.checkIn || submitting || locationLoading || (subscriptionData?.is_expired)}
            ...
            <div className="col-12 col-sm-6">
                <CButton
                    ...
                    disabled={!status.checkIn || status.checkOut || submitting || locationLoading || (subscriptionData?.is_expired)}
                    style={{ fontSize: '1rem', fontWeight: 'bold' }}
                >
                            ...
                            <CCard className="shadow-lg border-0">
                                ...
                                <CCardBody className="p-3 p-sm-4">
                                    ...
                                    <div className="d-block d-md-none">
                                        ...
                                        <ActionButtons />
                                        {/* Actions card strictly below action buttons on mobile */}
                                        <h5 className="mb-3 mb-sm-4 text-muted fw-bold fs-6">{t('LABELS.actions')}</h5>
                                        <div className="mb-3 mb-sm-4 p-3 p-sm-4 rounded-3 border-start border-info border-4" style={{ backgroundColor: '#e7f3ff' }}>
                                            <div className="d-flex align-items-center">
                                                <CIcon icon={cilLocationPin} className="me-2 me-sm-3 text-primary" size="lg" />
                                        ...
                                        {/* Calendar strictly below action buttons on mobile */}
                                        <div className="mb-4">
                                            <CalendarBlock />
                                        </div>
                                    </div>
